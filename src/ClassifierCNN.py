...
class ClassifierCNN:
    ...
    def train_the_model(self):
        ...
        model.add(keras.layers.Conv2D(filters=10, kernel_size=(8,50), activation='tanh', input_shape=self.data_training[0].shape, padding='same', kernel_regularizer=self.regularizer, bias_regularizer=self.regularizer))
        ...
        model.add(keras.layers.Dropout(0.2))
        model.add(keras.layers.Conv2D(filters=10, kernel_size=(2,10), activation='tanh', padding='same', kernel_regularizer=self.regularizer, bias_regularizer=self.regularizer))
        model.add(keras.layers.MaxPooling2D(pool_size=(2,2)))
